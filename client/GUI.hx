...
class GUI {
  ...
  private var shakeSL:Int;
  private var shakeNRG:Int;
  private var zoom:Float;
  private var zoomPhase:Int;
  private var zoomTarget:Int;
  
  public function spawn():Void {
    app.assetManager.getSound("player_connect").play();
    spawned = true;
    nextCheck();
  }
  ...
  public function render(bmp:Bitmap):Void {
    ...
    
    {
      ...
      if (Platform.mouse.y > Main.HEIGHT - 24){
        hoverAction = Zoom;
      }
      /*tooltips.push({
           x: 0
    ...
    if (!spawned){
      fontText.render(bmp, 120, Main.HEIGHT - 34, "You will spawn shortly!");
    }
    
    {
      ...
      var tt = {
          ...
          ,w: 32
          ,h: 24
          ,text: "Time until next turn: " + timeStr + "\nThe timer is currently set to " + FM.floor(GameState.period / 1000) + "s turns"
        };
      tooltips.push(tt);
      if (tooltipHover(tt) && GameState.SINGLE){
    ...
    
    {
      ...
      fontCounters.render(bmp, x + FM.prng.nextMod(shakeSL) + 13, 24 + 6, "" + GameState.currentSL);
      fontCounters.render(bmp, x + FM.prng.nextMod(shakeNRG) + 13, 48 + 6, GameState.currentNRG + "/" + GameState.maxNRG);
      if (shakeSL > 0){
        shakeSL--;
      }
      if (shakeNRG > 0){
      ...
      tooltips.push({
          ...
          ,w: 32
          ,h: 24
          ,text: "Starlight: " + GameState.currentSL + " (+" + (GameState.currentIncome + GameState.currentIngest) + ")" + "\nStarlight is the main resource.\nIt falls from the sky, so having\nmore land makes it more likely\nfor it to fall on your land."
        });
      tooltips.push({
           x: x
          ,y: 48
        ...
        if (!tooltipHover(guiBuildAreas[1])){
          ...
          for (i in 2...10){
            if (tooltipHover(guiBuildAreas[i])){
              disp = i;
              var okSL = i >= 3 && GameState.currentSL >= Unit.UNITS[i - 3].costSL;
              var okNRG = i >= 3 && GameState.currentNRG >= Unit.UNITS[i - 3].costONRG + Unit.UNITS[i - 3].costUNRG;
              if (okSL && okNRG){
                hoverAction = Build(i - 2);
              } else {
                hoverAction = Invalid(!okSL, !okNRG);
              }
              break;
            }
          }
    ...
    */
    ...
    if (zoomPhase != zoomTarget){
      ren.scale(Timing.quadInOut(zoomPhase / 100) * 90 + 100);
      zoomPhase += FM.signI(zoomTarget - zoomPhase);
    }
  ...
  public function click():Bool {
    var ret = (switch (hoverAction){
        ...
        switch (Unit.ORDERS[o].type){
          ...
          case Capture:
          var okSL = GameState.currentSL >= 1;
          var okNRG = GameState.currentNRG >= 1 + (GameState.faction[tileSelected] == -1 ? 1 : 0);
          if (okSL && okNRG){
            GameState.currentSL  -= 1;
            GameState.currentNRG -= (GameState.faction[tileSelected] == -1 ? 1 : 0);
            GameState.currentNRG -= 1;
            GameState.currentMoves.push(new Move(tileSelected, -1, Capture));
            GameState.turned[tileSelected] = MoveStatus.Moved;
            GameState.encodeTurnSend();
          } else {
            app.assetManager.getSound("invalid_move").play();
            if (!okSL){
              shakeSL = 5;
            }
            if (!okNRG){
          ...
          for (mi in 0...GameState.currentMoves.length){
            if (GameState.currentMoves[mi].from == tileSelected){
              switch (GameState.currentMoves[mi].type){
                case Capture:
                GameState.currentSL  += 1;
                GameState.currentNRG += (GameState.faction[tileSelected] == -1 ? 1 : 0);
                GameState.currentNRG += 1;
          ...
          var unit = GameState.unit[tileSelected];
          GameState.currentSL  += Unit.UNITS[unit].costSL;
          GameState.currentNRG += Unit.UNITS[unit].costONRG;
          GameState.currentNRG += Unit.UNITS[unit].costUNRG;
        ...
        GameState.encodeTurnSend();
        GameState.currentSL  -= Unit.UNITS[o - 1].costSL;
        GameState.currentNRG -= Unit.UNITS[o - 1].costONRG;
        GameState.currentNRG -= Unit.UNITS[o - 1].costUNRG;
        deselectTile();
        deselectAll();
        ren.rotate(camera);
        ordersTarget = 0;
    ...
    nextCheck();
    app.assetManager.getSound("gui_click").play();
    /*
    if (ret){
      app.assetManager.getSound("gui_click").play();
    }*/
  ...
  public function clickTile(t:Int):Void {
    ...
    if (cmd != null){
      if (cmd.type == Movement && GameState.unit[cmd.from] == 0){
        var okNRG = GameState.currentNRG >= 15;
        if (okNRG){
          GameState.currentNRG -= 15;
        ...
        if (cmd.type == Attack){
          var soundId = (switch (GameState.unit[cmd.from]){
              case 1: "sl_catcher_attack";
              case 2: "boot_attack";
              case 3: "bow_attack";
              case 4: "trebuchet_attack";
              case 5: "dynamite_attack";
              case 6: "dagger_attack";
              case _: "";
            });
          if (soundId != ""){
